...
| PB-019 | Email lead capture backend | Backend data hygiene | Minor | Submit or record a lead with casing and surrounding whitespace in the email address | Stored lead records, dedupe keys, and Mailchimp sync payloads should use the same canonical email | The public form trimmed before submit, but the backend persistence helper could still preserve whitespace/case if called directly by another server path | Failing integration test with `  Fan@Example.com  ` showed `lead.email` was saved with whitespace | Canonicalize once inside `recordEmailCaptureSubmission` and use that value for storage and connector sync | Fixed | `npm run test -- src/lib/__tests__/core.integration.test.ts`; `npm run test`; `node scripts/qa-smoke.mjs email-capture` |
| PB-020 | Public entry radius system | Design system consistency | Minor | Review the Backstage-themed home page, global error recovery actions, and shared brand mark | Public entry chrome should use the same Claude/Backstage `--r-*` primitive radius scale as the admin shell | Home process cards, global recovery controls, and the shared brand mark still used raw `rounded-[7px]`, `rounded-[9px]`, `rounded-[10px]`, `rounded-[14px]`, or `rounded-2xl` classes | Source scan plus failing public entry radius-token regression | Replace raw public entry radii with `rounded-[var(--r-sm)]`, `rounded-[var(--r-md)]`, and `rounded-[var(--r-lg)]` by component role | Fixed | `npm run test -- src/app/__tests__/admin-theme-tokens.test.ts`; `rg -n "rounded-\\[(?:7px|9px|10px|14px|20px)\\]|rounded-2xl|rounded-xl|rounded-lg|rounded-md|rounded-sm" src/app/page.tsx src/app/global-error.tsx src/components/brand/brand-mark.tsx` |
| PB-021 | Admin chrome radius system | Design system consistency | Minor | Review the Backstage admin shell, overview, settings chrome, preview chrome, lead inbox, public-link panel, mobile menu, import flow, editor status icons, artwork crop modal, and form-state alerts | Admin chrome should use the Claude/Backstage `--r-*` primitive radius scale rather than one-off raw radius classes | Admin chrome still mixed `rounded-[7px]`, `rounded-[8px]`, `rounded-[9px]`, `rounded-[10px]`, `rounded-[14px]`, `rounded-[20px]`, and `rounded-2xl` across cards, menus, buttons, alerts, status icons, and modal surfaces | Source scan plus failing admin radius-token regression | Replace raw admin chrome radii with `rounded-[var(--r-sm)]`, `rounded-[var(--r-md)]`, `rounded-[var(--r-lg)]`, and `rounded-[var(--r-xl)]` by component role | Fixed | `npm run test -- src/app/__tests__/admin-theme-tokens.test.ts`; admin raw-radius `rg` scan |
| PB-022 | Full-radius token usage | Design system consistency | Minor | Review Backstage pills, avatars, toggles, circular meters, progress bars, and brand lockup domain chips | Pill and circular chrome should use the defined Claude/Backstage `--r-full` primitive instead of Tailwind's raw `rounded-full` alias | Admin and brand chrome still used raw `rounded-full`, bypassing the same radius-token contract enforced for smaller card and control radii | Failing radius-token regression showed 20 raw `rounded-full` admin usages plus the brand lockup full-radius chip | Replace raw `rounded-full` with `rounded-[var(--r-full)]` in Backstage/admin chrome and the shared brand lockup | Fixed | `npm run test -- src/app/__tests__/admin-theme-tokens.test.ts`; Backstage raw `rounded-full` scan |
| PB-023 | High-contrast action foregrounds | Design system consistency | Minor | Review primary admin actions, shared button variants, avatar initials, the Spotify import affordance, global recovery actions, and shared brand mark in light and dark Backstage themes | High-contrast text on accent/brand fills should use the Claude `--text-on` primitive through the app token layer | Admin, shared UI, and public-entry chrome still used raw `text-white` / `active:text-white`, bypassing theme ownership for on-fill foreground contrast | Failing foreground-token regression required `--app-text-on` and rejected raw `text-white` classes in Backstage chrome sources | Map `--app-text-on` to `--text-on` inside the Backstage theme and use `text-[var(--app-text-on)]` for high-contrast foregrounds | Fixed | `npm run test -- src/app/__tests__/admin-theme-tokens.test.ts`; Backstage raw `text-white` scan |
| PB-024 | Specialized admin radius tokens | Design system consistency | Minor | Review analytics swatches, lead inbox table chrome, artwork cropper, preview-not-found dialog, and admin device preview frames | Any specialized radius that preserves a design-file shape should still be owned by the Backstage `--r-*` token layer | The radius regression missed arbitrary bracket classes, leaving raw `rounded-[3px]`, `rounded-[12px]`, `rounded-[16px]`, `rounded-[18px]`, `rounded-[26px]`, and `rounded-[34px]` in admin chrome | Tightened radius-token regression failed with nine raw arbitrary radius classes | Add semantic `--r-*` tokens for swatches, framed media, cropper, dialogs, and device preview chrome, then replace the raw bracket classes | Fixed | `npm run test -- src/app/__tests__/admin-theme-tokens.test.ts`; admin raw arbitrary-radius scan |
